...
class ART_block(nn.Module):
    def __init__(self, config, input_dim, img_size=224, transformer=None):
        ...
        if self.transformer:
            ...
            # Upsampling block - 64x64 -> 256x256로 만들기 위해 2번의 추가 upsampling
            model = [
                # First upsampling: hidden_size -> ngf * 8
                nn.ConvTranspose2d(self.config.hidden_size, ngf * 8,
                                kernel_size=3, stride=2,
                                padding=1, output_padding=1,
                                bias=use_bias),
                norm_layer(ngf * 8),
                ...
                # Third upsampling: ngf * 4 -> ngf * 4
                nn.ConvTranspose2d(ngf * 4, ngf * 4,
                                kernel_size=3, stride=2,
                                padding=1, output_padding=1,
                                bias=use_bias),
                norm_layer(ngf * 4),
    ...
    def forward(self, x):
        if self.transformer:
            ...
            # Downsample
            down_sampled = self.downsample(x)
            # print(f"After downsample: {down_sampled.shape}")
            
            # Embed
            embedding_output = self.embeddings(down_sampled)
            # print(f"After embedding: {embedding_output.shape}")
            ...
            transformer_out = transformer_out.permute(0, 2, 1)
            transformer_out = transformer_out.contiguous().view(B, hidden, h, w)
            # print(f"After reshape: {transformer_out.shape}")
            
            # Upsample
            transformer_out = self.upsample(transformer_out)
            # print(f"After upsample: {transformer_out.shape}")
