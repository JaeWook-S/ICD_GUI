...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        if padding_type == 'reflect': # 현재 reflect 사용
...
class ResViT(nn.Module):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            posemb = np2th(weights["Transformer/posembed_input/pos_embedding"])# ?? : original GFP는 노이즈가 많아서 줄였음, RFP는 노이즈가 너무 없어서 늘렸음?? (반대 작업)
...
class Res_CNN(nn.Module):
    def __init__(self, config, input_dim, img_size=224, output_dim=3, vis=False):
        ...
        ############################################################################################
        # Layer1-Encoder1
        model = [nn.ReflectionPad2d(3), # 반사 패딩 적용 -> 좌우상하 3픽셀씩 추가
                 nn.Conv2d(input_dim, ngf, kernel_size=7, padding=0, # 패딩 적용해서 입력 크기 동일
                           bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        setattr(self, 'encoder_1', nn.Sequential(*model))
        ...
        model = [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                    kernel_size=3, stride=2,
                                    padding=1, output_padding=1,
                                    bias=use_bias), # h,w 가 2배로 늘어나고 채널은 절반으로 줄음
                 norm_layer(int(ngf * mult / 2)),
                 nn.ReLU(True)]
        setattr(self, 'decoder_1', nn.Sequential(*model))
        ############################################################################################
...
CONFIGS = {
    ...
    'Res-ViT-B_16': configs.get_resvit_b16_config(), # 현재 이거 사용
